<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-white">Новий захід</h2>
    </x-slot>
    ...
    <div class="max-w-3xl mx-auto px-4 py-8">
        ...
        <form method="POST" action="{{ route('organizer.events.store') }}" enctype="multipart/form-data"
              x-data="{ imageError: '', isOnline: {{ old('is_online') ? 'true' : 'false' }} }"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <x-input-label for="image" value="Фото (необов'язково)" />
                    <input id="image" name="image" type="file" accept="image/jpeg,image/png,image/gif,image/webp"
                           class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-indigo-500/20 file:text-indigo-400 hover:file:bg-indigo-500/30"
                ...
                <div>
                    <x-input-label for="price" value="Ціна (грн, 0 = безкоштовно) *" />
                ...
                <div x-show="!isOnline" x-cloak>
                    ...
                    <x-text-input id="location" name="location" type="text" class="mt-1 block w-full"
                                  value="{{ old('location') }}" placeholder="Наприклад: Палац Україна"
                                  x-bind:required="!isOnline" />
                    <x-input-error :messages="$errors->get('location')" class="mt-1" />
                </div>

                <div x-show="!isOnline" x-cloak>
                    <x-input-label for="address" value="Адреса (необов'язково)" />
                    <x-text-input id="address" name="address" type="text" class="mt-1 block w-full"
                                  value="{{ old('address') }}" placeholder="вул. Велика Васильківська, 103" />
                </div>

                <!-- Map picker -->
                <div class="md:col-span-2" x-show="!isOnline" x-cloak>
                    <x-input-label value="Місце на карті (необов'язково)" />
                    <p class="text-xs text-gray-500 mb-2 mt-0.5">Натисніть на карту, щоб позначити місце проведення</p>
                    <div id="picker-map" class="w-full rounded-xl border border-gray-700 isolate" style="height:280px;"></div>
                    <p id="coords-hint" class="text-xs text-gray-500 mt-1">
                        @if(old('latitude')) {{ old('latitude') }}, {{ old('longitude') }} @else Позначку не встановлено @endif
                    </p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const isOnline = {{ old('is_online') ? 'true' : 'false' }};
            if (isOnline) return;

            const initLat = @json(old('latitude'));
            const initLng = @json(old('longitude'));

            const map = L.map('picker-map').setView(
                initLat ? [initLat, initLng] : [50.4501, 30.5234], 12
            );
